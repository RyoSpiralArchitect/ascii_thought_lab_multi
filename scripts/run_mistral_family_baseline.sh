...
MODELS=(
  "${MODEL_1-mistral-large-latest}"
  "${MODEL_2-mistral-medium-latest}"
  "${MODEL_3-mistral-small-latest}"
)
...
RUN_DIR="${RUN_ROOT%/}/mistral_family_baseline_${STAMP}"
...
SWAP_BANK="${SWAP_BANK:-}"
INSUFFICIENT_PATTERNS="${INSUFFICIENT_PATTERNS:-insufficient[_ ]quota|current quota|credit balance|out of credits|payment required}"
...
for model in "${MODELS[@]}"; do
  for problem in "${PROBLEMS[@]}"; do
    ...
    if ! "${CMD[@]}" 2>&1 | tee "$RUN_LOG"; then
      ...
      if rg -qi "$INSUFFICIENT_PATTERNS" "$RUN_LOG"; then
        ABORT_REASON="insufficient_quota_or_credit"
        echo "ABORTING remaining runs: Mistral quota or credits appear exhausted." >&2
        break 2
      fi
    fi
  done
